...
let
  cfg = config.services.myrabbitmq;
  ...
  config_env_file_content = lib.generators.toKeyValue { } cfg.configEnvs;
  config_env_file = pkgs.writeText "rabbitmq-env.conf" config_env_file_content;
...
in
{
  ...
  imports = [
    (lib.mkRemovedOptionModule [ "services" "myrabbitmq" "cookie" ] ''
  ...
  options = {
    services.myrabbitmq = {
      ...
      configEnvs = lib.mkOption {
        ...
        '';
        description = ''
          rabbitmq's environment variables configuration.
          This is known as the `rabbitmq-env.conf` file,
          which contains some configuration set by environment variables.

          See https://www.rabbitmq.com/configure.html#customise-environment
        '';
      };
  ...
  config = lib.mkIf cfg.enable {
    ...
    services.myrabbitmq.configItems =
      {
    ...
    services.myrabbitmq.configEnvs =
      {
        "NODENAME" = cfg.nodename;
      };

    services.myrabbitmq.plugins = lib.optional cfg.managementPlugin.enable "rabbitmq_management";

    systemd.services.myrabbitmq = {
      description = "rabbitmq Server(adjusted by chenjf)";
      ...
      environment = {
        ...
        '';
        RABBITMQ_NODENAME = cfg.nodename;
      } // lib.optionalAttrs (cfg.config != "") { RABBITMQ_ADVANCED_CONFIG_FILE = advanced_config_file; }
      // lib.optionalAttrs (cfg.configEnvs != {}) { RABBITMQ_CONF_ENV_FILE = config_env_file; };
